...
mpl.rcParams['axes.linewidth']='2'#FOR PNG=2 #'3'
...
def multi_mplot(x, y, label_x= 'x', label_y='y', outFile='FreeEnergy_versus_P'+ext, Title='', col1='r.', col2='b.', legends=None, legend_loc=2, N=111):
		...
		mplt.plot(x,y, col1, linewidth=6, markersize=marker_size, markeredgewidth=(markeredgewidth_ratio)*font_size)
		#mplt.plot(x,y, col2)		
		mplt.xlabel(label_x)
		mplt.ylabel(label_y)
		mplt.title(Title)
...
def plot_surf(x,y,z, xlabel='', ylabel='', zlabel='', outFile='surf'+ext, N=111):
	...
	ax.plot_surface(x, y, z, cmap=mplt.cm.hot, rstride=1, cstride=1, linewidth=6, antialiased=False)#flag)#coolwarm)
	#ax.set_zlim(0, 1)
...
#			struc_ids_AB, FreeEnergies_AB, Probabilities_AB			= read_all_substruct_data(inFile=path+'all_substruct_data', outPath=path)
		...
		# subplot
		outFile = path+'transLen_versus_P_BB_subplot'+'.png'#ext
		for i in range(len(P_trascLen_BB)):
		...
		outFile = path+'transLen_versus_P_BB_AB_subplot'+'.png'#ext
		for i in range(len(P_trascLen_BB)):
...
?		f=mplt.gcf()
		...
		# subplot
		outFile = path+'transLen_versus_P_AB_subplot'+'.png'#ext
		for i in range(len(P_trascLen_AB)):
