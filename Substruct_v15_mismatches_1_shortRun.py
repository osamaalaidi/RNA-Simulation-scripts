...
def read_tables():
	...
	number_of_P1_bps_list	=	[]
	number_of_AT_bps_list	=	[]
	...
	for i in range(len(substructs_seqs)):
		...
		if i==0:
			...
			construct,P1_helix_indix_1,P1_helix_indix_2,P1_sequence,P1_structure,AT_helix_indix_1,AT_helix_indix_2,AT_sequence,AT_structure,Mg,AT_structure_slip,number_of_P1_bps,number_of_AT_bps	=	substruct_seqs.split('\t')
			...
			number_of_P1_bps_list.append(int(number_of_P1_bps))
			number_of_AT_bps_list.append(int(number_of_AT_bps))
	...
	substrucs_data	= {
						...
						'number_of_P1_bps_list':number_of_P1_bps_list,
						'number_of_AT_bps_list':number_of_AT_bps_list
...
if __name__=='__main__':
	...
	seqFilePath			=	'seq.fasta'
	logFileName			=	'report.log'
	logFile				= 	open(logFileName,'w')
	mismatches			= 0 # number of allowed mismatches in AT base pairing
	min_AT				= 4 # minimum AT length for an aptamer-containing structure to be considered as an intermediate, hence not classified an aptamer.
	full_P1				= 3 # full P1 length 
	################################################################################
	...
	number_of_P1_bps_list	= substrucs_data['number_of_P1_bps_list']
	number_of_AT_bps_list	= substrucs_data['number_of_AT_bps_list']
	...
	for i in range(len(substrucs_Names)):
		...
		print 'sequence:' ,P1_seq_1,P1_seq_2
		print 'actual', seq[int(P1_start_index_1)-1: int(P1_end_index_2) ]

		# AT search
		AT_substrucs[i]						= AT_substrucs[i].strip(' ')
		AT_1_start, AT_2_end				= AT_substrucs[i].split('*')
		AT_start_index_1, AT_end_index_1	= AT_start_end_index_1[i].split(':')
		AT_start_index_2, AT_end_index_2	= AT_start_end_index_2[i].split(':')

		# Accounting for AT slippage
		AT_1_start_slip, AT_2_end_slip		= AT_substrucs_slip[i].split('*')

		## AT Sequence check
		AT_seq_1,AT_seq_2 					= AT_sequence_list[i].split('*')
		AT_seq_pattern 						= re.compile(AT_seq_1+'.*'+AT_seq_2)
		AT_seq_index						= AT_seq_pattern.match(seq[ int(AT_start_index_1)-1: int(AT_end_index_2)] )

		print >> logFile, 'Sequence:'
		print >> logFile, seq[int(P1_start_index_1)-1: int(P1_end_index_2) ], ' , ', seq[int(AT_start_index_1)-1: int(AT_end_index_2)]
		...
		for j in range(len(strucs)):
			...
			P1_index_1=		P1_1_start	== strucs[j][int(P1_start_index_1)-1: int(P1_end_index_1)]
			P1_index_2=		P1_2_end	== strucs[j][int(P1_start_index_2)-1: int(P1_end_index_2)]

			#print P1_1_start, strucs[j][full_P1-int(P1_end_index_1): full_P1-int(P1_end_index_1)+len(P1_1_start) ]
			#print P1_2_end, strucs[j][int(P1_start_index_2)-1: int(P1_end_index_2)]
			# Bracket count condition
			P1_count_index =strucs[j][int(P1_start_index_1)-1: int(P1_end_index_2)].count('(') == strucs[j][int(P1_start_index_1)-1: int(P1_end_index_2)].count(')')
			#print strucs[j][full_P1-int(P1_end_index_1): int(P1_end_index_2)].count('(')
			#print strucs[j][full_P1-int(P1_end_index_1): int(P1_end_index_2)].count(')')
			...
			if AT_count_index ==True:
				...
				if 'Aptamer' in substrucs_Name:
					...
					AT_len= number_of_AT_bps_list[i]
			...
			#print j,  P1_index_1,P1_index_2 ,AT_index_1 ,AT_index_2 ,P1_count_index ,AT_count_index ,P1_seq_index ,AT_seq_index


			#	P1_seq_index and AT_seq_index should be ignored for mutants
			...
			print >> logFile2, j, '\t', strucs[j][int(P1_start_index_1)-1: int(P1_end_index_2) ],' , ', strucs[j][int(AT_start_index_1)-1: int(AT_end_index_2)], s 
	...
	for i in range(len(all_indecies)):
		...
		FreeEnergies_with_Substrucst_all	= np.where( all_indecies[i], all_freeEnergies[i], a )
	...
	for i in range(len(substrucs_Names)):
		...
		indicies_without_Substrucst_before_final= all_indecies[i] + indicies_without_Substrucst_before_final
		indicies_without_Substrucst_after_final	= all_indecies[i] + indicies_without_Substrucst_after_final
